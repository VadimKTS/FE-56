...
function renderAllCards(cards) {
  ...
  cards.forEach(card => {
    cardList.innerHTML += `<li class="card">
    <div class="cadr-body">
      <h3>${card.title}</h3>
    ...
    <div class="card-buttons">
      <button class="edit" id="openModal">EDIT</button>
...
users.then(users => {
  ...
  open.addEventListener('click', () => {
    ...
    confirm.addEventListener('click', () => {
      ...
      cardList[indexCard] = card; // Обновляем данные
...
users.then(users => {
  ...
  console.log(modal);
  const open = document.getElementById('addTodoModal');
  open.addEventListener('click', () => {
    modal.render();

    const confirm = document.querySelector('.add-card-modal-button-confirm');
    confirm.addEventListener('click', () => {
      const title = document.getElementById('title').value;
      const description = document.getElementById('description').value;
      const select = document.getElementById('select');
    ...
    const close = document.querySelector('.add-card-modal-button-cancel');
    close.addEventListener('click', () => modal.remove());
  });
});
